...
export function getRiskProfile(riskPercent: number): keyof typeof PROFITPROS_BRAND.messages {
  if (riskPercent <= 2) return 'conservative';
  if (riskPercent <= 3) return 'balanced';
  return 'aggressive';
}
...
export function calculatePsychologyScore(params: {
  ...
  return (
    params.winRate * weights.winRate +
...
export function updateStreak(
  currentStreak: TradingStreak,
  todayPerformance: TradingStreak['habits']
): TradingStreak {
